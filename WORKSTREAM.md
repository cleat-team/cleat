...
| Database ports (`:80`) — WS-3 is Postgres `5434`, MySQL `3308`, SQL Server `1435` | **live.** Check it with the skip list, not a skip *count*: `go test ./engine/ -count=1 -json` with all three `CLEAT_TEST_*` set must report **`FAIL=0`**, and every skip must be a toolchain-absent Python component test (`componentize-py` cannot run on macOS — see `scripts/docker/python-toolchain.Dockerfile`). A dialect that did not connect shows up as failures, not skips, which is why `FAIL=0` is the load-bearing half. **The count is not stable and this cell used to pin one.** It said `skip=4` on 2026-09-03 and was 6 by 2026-09-04, because #667 added two Python tests — a number that moves whenever a toolchain-gated test is added, which is exactly the rot this table keeps collecting. It previously argued from wall clock ("133s, and Postgres-only is ~21s"), the inference CLAUDE.md retired: a no-DSN run now takes ~150s and clears its own threshold while testing no database at all. |
