...
// is taken before the host is ever consulted. IMPROVEMENT-PLAN 3.95 moves it
...
func TestBothSDKsAgreeOnTheHostRetryBudget(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("reading %s: %v\n\n"+
			...
			"unless the threshold itself has moved host-side (3.95), which is "+
	...
	if rustMs != goMs {
		t.Fatalf("the two SDKs disagree about the host-retry threshold: "+
			...
			"host-side per IMPROVEMENT-PLAN 3.95 and delete both constants.",
