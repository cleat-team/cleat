...
| | CleatClaim | CleatRunLifecycle | QueueAdmission | CleatDurableCallIntent | Signals | StateMachine |
|---|---|---|---|---|---|---|
| Parsed by SANY | **Yes** | **Yes** | **Yes** | **Yes** | No | No |
| Checked by TLC | **Yes** (cleat#2034 fixed its vacuity) | **Yes** | **Yes** | **Yes** | No | No |
| `.cfg` exists | **Yes** | **Yes** | **Yes** | **Yes** | No | No |
| Run in CI | **Yes**, on touching PRs | **Yes**, on touching PRs | **Yes**, on touching PRs | **Yes**, on touching PRs | No | No |
| Maintained | **Yes** | **Yes** | **Yes** | **Yes** | **No — superseded, see above** | **No — superseded, see above** |
...
## `CleatDurableCallIntent.tla` (cleat#1999)
...
both found real counterexamples (see that spec's section above), and per the policy the fix
is not landed in this PR. The counterexample traces are filed as a tracked issue (the
