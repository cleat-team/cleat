...
func TestTwoTenantsCanHoldTheSameConcurrencyKey(t *testing.T) {
	...
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			t.Cleanup(func() {
				_, _ = sa.ReleaseConcurrencyKey(context.Background(), key, runA)
				_, _ = sb.ReleaseConcurrencyKey(context.Background(), key, runB)
			})
			...
			if _, err := sa.ReleaseConcurrencyKey(ctx, key, runA); err != nil {
