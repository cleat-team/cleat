...
// reapEverythingNow is the sweep window this test uses, and it is NEGATIVE on
// purpose. cleat#1447.
//
// THE TEST USED TO PASS 0 HERE AND THAT COUPLED IT TO A CLOCK TICK.
// reapStaleInstancesOnce sweeps on
//
//	heartbeat_at < DATEADD(SECOND, @p1, SYSUTCDATETIME())   -- @p1 = -int(timeout.Seconds())
//
// so a timeout of 0 makes the threshold EXACTLY SYSUTCDATETIME(). A claim sets
// heartbeat_at = SYSUTCDATETIME(). If both land in one tick the predicate is
// false and nothing is reclaimed -- which is "reclaimed 0", the failure that got
// this test reverted in #1448.
//
// Measured on mcr.microsoft.com/mssql/server:2022-latest:
//
//	20 rapid SYSUTCDATETIME() calls, one batch   ->  1 distinct value
//	spin until the value changes                 ->  4104 microseconds
//	heartbeat write + predicate, one batch, x200 ->  FALSE 198 / TRUE 2
//
// The clock ticks roughly every 4 ms, and the only thing that saved the test
// locally was the Go round-trip between the two statements being longer than
// that -- 12/12 reclaimed here with timeout 0. A margin of a few milliseconds is
// exactly what differs between a laptop and a loaded runner.
//
// A negative timeout puts the threshold in the FUTURE, so the predicate holds
// however the two statements fall against the tick. What this test is actually
// about is the STATUS arm -- a running row with a stale heartbeat is reclaimed,
// the same row parked is not -- and the timeout was always incidental to that.
//
// NOT A PRODUCT CHANGE. Production reaps with timeouts of seconds to minutes,
// where a 4 ms granularity cannot matter. reapStaleInstancesOnce is untouched.
const reapEverythingNow = -time.Second
...
func TestTheReaperDoesNotReclaimASleepingWorkflow(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			n1, err := store.ReapStaleInstances(ctx, reapEverythingNow, 100)
			if err != nil {
			...
			n2, err := store.ReapStaleInstances(ctx, reapEverythingNow, 100)
			if err != nil {
