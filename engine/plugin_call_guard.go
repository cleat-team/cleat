...
func (g *PluginCallGuard) Check(callerName, targetName string) error {
	...
	return fmt.Errorf("plugin %q is not allowed to call plugin %q. Add %q to the caller's call_plugin capability in its manifest.", callerName, targetName, targetName)
}
