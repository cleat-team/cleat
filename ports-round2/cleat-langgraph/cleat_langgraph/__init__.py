...
2. **Workflow Layer** (inside WASM sandbox): Cleat ``@cleat_entry`` functions
   that orchestrate the LangGraph execution via ``HostCalls.cleat_call()``.
...
- **Per-node durability**: Each LangGraph node/task is a separate
  ``cleat_call``, matching the Temporal plugin's per-activity granularity.
- **Stateless host service**: All execution state is passed through the
  ``cleat_call`` request/response cycle. The host caches compiled graphs
