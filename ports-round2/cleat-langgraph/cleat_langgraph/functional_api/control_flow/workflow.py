...
per-task durability. Each ``durable_call`` is recorded as a separate
...
Key difference from the Temporal version:
  ...
  - Cleat: ``durable_call`` is synchronous. Items are validated sequentially
...
@durable_entry(name="control_flow")
def control_flow_workflow(h: HostCalls, items: list[str]) -> dict:
    ...
    Workflow:
    1. Validate each item (sequential durable_calls).
    ...
    h.durable_log(f"{len(valid_items)} valid items out of {len(items)}")
