...
def chatbot_functional_workflow(h: HostCalls, user_message: str) -> str:
    ...
    """
    h.durable_log("Generating draft...")
    draft_result = h.durable_call(
        "langgraph",
        "execute_task",
    ...
    h.durable_log(f"Draft: {draft[:60]}...")
    h.set_query_state("draft", draft)

    # Wait for human feedback
    h.durable_log("Waiting for human_feedback signal...")
    ...
    h.durable_log(f"Feedback: {feedback}, approved: {approved}")

    # Process review
    review_result = h.durable_call(
        "langgraph",
        "execute_task",
        {
    ...
    h.durable_log("Chatbot functional workflow complete")
