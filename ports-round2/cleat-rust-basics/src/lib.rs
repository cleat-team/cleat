...
//!
//! Each `#[durable_entry]` function is compiled as a separate WASM export.
...
// since durable_sleep takes i64 milliseconds.
...
//
// Ported from: p0_durable_execution.rs
// Restate used: ctx.run() for idempotent actions, ctx.rand_uuid() for idempotency keys
// Cleat uses:   durable_call() for durable RPC, uuid(seed) for deterministic UUIDs
//
// GAPS:
//   - No `ctx.run()` equivalent: Restate wraps side effects in `ctx.run()` to
//     persist results and skip re-execution on replay. Cleat's durable_call() is
...
fn subscription_add(h: &HostCalls, req: SubscriptionRequest) -> Result<String, String> {
    ...
    h.durable_log(&format!("Generated payment idempotency key: {}", payment_id));

    // ---- Create recurring payment ----
    // In Restate: ctx.run(|| create_recurring_payment(&req.credit_card, &payment_id)).await?
    // In Cleat: durable_call("service", "operation", request_json)
    let payment_input = serde_json::json!({
        "credit_card": req.credit_card,
        "payment_id": payment_id,
    });
    let (pay_ref, err) = h.durable_call(
        "payment-service",
    ...
    h.durable_log(&format!("Created recurring payment: {}", pay_ref));
    ...
    for sub in &req.subscriptions {
        ...
        if let Some(e) = err {
            ...
            h.durable_log(&format!("Failed to create subscription '{}': {} (continuing)", sub, e));
            continue;
        }
        h.durable_log(&format!("Created subscription '{}': {}", sub, result));
    }
...
fn building_blocks_demo(h: &HostCalls, _input: EmptyInput) -> Result<String, String> {
    h.durable_log("=== Building Blocks Demo ===");

    // ---- 1. Durable RPC (call another service) ----
    // Restate: ctx.object_client::<X>("key").create("req").call()
    // Cleat:   durable_call("service", "operation", json_payload)
    let (result, err) = h.durable_call(
        "subscription-service",
        "get_status",
        "{\"dummy\": true}",
    ...
    if let Some(e) = err {
        h.durable_log(&format!("RPC call failed (expected in isolation): {}", e));
    } else {
        h.durable_log(&format!("RPC result: {}", result));
    }

    // ---- 2. Durable Timers (sleep) ----
    // Restate: ctx.sleep(Duration::from_secs(5)).await?
    // Cleat:   h.durable_sleep(5000) -- panics with SuspendSentinel when suspending
    // Note: In the WASM runtime, durable_sleep causes the workflow to suspend
    // and be resumed later. In test/unit context, this will suspend.
    h.durable_log("Would sleep for 5 seconds (suspends in WASM runtime)");
    ...
    if let Some(e) = err {
        h.durable_log(&format!("Create promise failed: {}", e));
    } else {
        h.durable_log(&format!(
    ...
    match signal_err {
        Ok(()) => h.durable_log("Signal sent successfully"),
        Err(e) => h.durable_log(&format!("Signal send failed (expected in isolation): {}", e)),
    }
    ...
    h.durable_log(&format!("Deterministic UUID: {}", id));

    // ---- 6. Random number ----
    // Restate: ctx.rand_uuid() (randomness)
    // Cleat:   h.random() -> i64
    let r = h.random();
    h.durable_log(&format!("Random (deterministic replay): {}", r));

    // ---- 7. Version info ----
    let version = h.version();
    let min_version = h.min_version();
    h.durable_log(&format!("Host version: {}, min_version: {}", version, min_version));
    ...
    if cancelled {
        h.durable_log(&format!("Cancellation requested: {}", reason));
        return Ok(serde_json::json!({"status": "cancelled", "reason": reason}).to_string());
    }
    h.durable_log("Not cancelled");
    ...
    h.durable_log(&format!("Previous scope: '{}'", prev));

    let (obj_type, inst_key) = h.get_scope();
    h.durable_log(&format!("Current scope: type='{}', key='{}'", obj_type, inst_key));
...
fn greeter_ungreet(h: &HostCalls, input: GreetInput) -> Result<String, String> {
    ...
    h.durable_log(&format!("GreeterObject '{}': ungreet called (state not persisted)", inst_key));
...
//               durable_call() for service calls, get_scope() for key
//
// GAPS:
...
fn signup_run(h: &HostCalls, user: User) -> Result<String, String> {
    ...
    h.durable_log("User entry created");
    ...
    h.durable_log(&format!("Verification secret generated: {}", secret));
    ...
    let (email_result, err) = h.durable_call(
        "email-service",
        "send_verification",
    ...
    h.durable_log(&format!("Email sent: {}", email_result));
    ...
    h.durable_log("Awaiting email verification signal...");
    ...
    if timed_out {
        h.durable_log("Verification timed out (24h)");
    ...
    h.durable_log(&format!("Received signal '{}' with payload: {}", signal_name, payload));

    // ---- Step 4: Verify the secret matches ----
    let matched = payload.trim_matches('"') == secret;
    if matched {
        h.durable_log("Email verified successfully! Signup complete.");
    } else {
        h.durable_log("Verification secret mismatch.");
    }
...
fn signup_click(h: &HostCalls, input: ClickInput) -> Result<String, String> {
    ...
    match result {
        Ok(()) => {
            h.durable_log("Verification signal sent to workflow");
            Ok(serde_json::json!({"status": "signal_sent", "user_id": input.user_id}).to_string())
        }
        Err(e) => {
            h.durable_log(&format!("Failed to send verification signal: {}", e));
...
// "key-value-store" service callable via durable_call. This demonstrates
...
/// Set a value in the key-value store (workaround via durable_call).
...
fn kv_set(h: &HostCalls, store: &str, key: &str, value: &str) -> Result<(), String> {
    ...
    let (_resp, err) = h.durable_call("key-value-store", "set", &input.to_string());
    match err {
        None => Ok(()),
...
/// Get a value from the key-value store (workaround via durable_call).
/// Requires a "key-value-store" service to be registered with the host.
#[allow(dead_code)]
fn kv_get(h: &HostCalls, store: &str, key: &str) -> Result<Option<String>, String> {
    let input = serde_json::json!({"store": store, "key": key});
    let (resp, err) = h.durable_call("key-value-store", "get", &input.to_string());
    match err {
        None => {
...
mod tests {
    ...
    /// Test that function signatures are compatible with #[durable_entry]
    #[test]
    fn test_durable_entry_fn_signature() {
