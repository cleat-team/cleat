...
| `@Handler` method | `@DurableEntry` public static method with `HostCalls` as first param. |
...
| `AccountClient.fromContext()` then `.withdraw().await()` | `HostCalls.durableCall("account", "withdraw", json)` |
...
cleat-java-state-machine/
  settings.gradle.kts          -- Multi-project: includes :durable-java
  ...
  src/main/java/cleatexample/statemachine/
    ...
    PaymentProcessor.java      -- Three @DurableEntry workflows
    Account.java               -- Three @DurableEntry account operations
    types/
...
- **makePayment**: NEW -> COMPLETED_SUCCESSFULLY (on success). Stores status and
  payment details in query state. Calls `account:withdraw` via durableCall.
