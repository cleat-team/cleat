...
def book_travel(h: HostCalls, request: BookingRequest) -> BookingResult:
    ...
    h.cleat_log(f"Starting travel booking for user {request.user_id}")
    ...
    try:
        ...
        h.cleat_log(f"Flight reserved: {result.flight_reservation_id}")
    except Exception as e:
        h.cleat_log(f"Flight reservation failed: {e}")
    ...
    try:
        ...
        h.cleat_log(f"Hotel reserved: {result.hotel_reservation_id}")
    except Exception as e:
    ...
    try:
        ...
        h.cleat_log(f"Payment processed: {result.payment_charge_id}")
    except Exception as e:
        # Compensate: cancel flight + hotel
        h.cleat_log(f"Payment failed: {e}. Compensating all...")
        h.cleat_call("flights", "Cancel", {"reservation_id": result.flight_reservation_id})
        h.cleat_call("hotels", "Cancel", {"reservation_id": result.hotel_reservation_id})
        result.status = f"failed: {e}"
        return result

    result.status = "confirmed"
    h.cleat_log(f"Travel booking complete for user {request.user_id}")
    return result
