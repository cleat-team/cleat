...
def process_item(h: HostCalls, item: str) -> str:
    ...
    h.durable_log(f"Processing item: {item}")
    response = h.durable_call("warehouse", "ProcessItem", {"item": item})
    return response
...
def process_order(h: HostCalls, input: OrderInput) -> str:
    ...
    h.durable_log(f"Processing order {input.order_id} with {len(input.items)} items")
    ...
    for item in input.items:
        ...
        h.durable_log(f"Started child workflow for {item}: {run_id}")

    # Await all children
    results: list[ChildResult] = h.await_all_children(run_ids)

    h.durable_log(f"All items processed for order {input.order_id}")
    return json.dumps({
