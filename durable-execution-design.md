# Durable Execution on an Elastic Cluster
...
When a workflow function needs an operation that interacts with the outside world or with time, it MUST use the `*durable.HostCalls` API rather than the corresponding standard library function:
...
Every function that directly or transitively calls a durable function MUST accept `*durable.HostCalls` as its first parameter. This requirement propagates through the entire call graph:

```go
func validateAndReserve(h *durable.HostCalls, userID string, items []Item) (Reservation, error) {
    // calls h.DurableCall(...) internally
    return lookupUser(h, userID, items)
}

func lookupUser(h *durable.HostCalls, userID string, items []Item) (Reservation, error) {
...
A `durable vet` command statically detects potential non-determinism sources before compilation:
...
```go
func ApprovalWorkflow(h *durable.HostCalls, orderID string) (string, error) {
...
```go
// In the durable runtime (imported by workflow code):
type HostCalls struct {
    DurableCall  func(service, operation, requestJSON string) (responseJSON string, err error)
...
```go
func OrderWorkflow(h *durable.HostCalls, userID string, cart []CartItem) (string, error) {
...
```
durable schedules list                                 -- list all schedules
durable schedules get <name>                           -- show schedule details
durable schedules create <name> --def <def>            -- create a new schedule
    --cron "0 * * * *" --tz "America/New_York"
    --input '{"key": "val"}' --max-concurrent 1
    --timeout 3600 --retry '{"max_attempts": 3}'
    --misfire-threshold 300
    --catch-up immediate
durable schedules update <name> --cron "*/30 * * * *"  -- update schedule fields
durable schedules delete <name>                         -- remove a schedule
durable schedules pause <name>                          -- set enabled = false
durable schedules resume <name>                         -- set enabled = true
durable schedules trigger <name> [--input '...']        -- fire immediately (ad-hoc)
```

**`durable schedules trigger`** fires the schedule immediately regardless of `next_fire_at` or `enabled` status. It:
...
**`durable schedules create`** validates the cron expression on the client side before inserting the row. The server also validates on write (see 8.11.10).
...
1. **CLI/API (write time).** The `durable schedules create` and `durable schedules update` commands validate the cron expression using `robfig/cron`. If the expression does not parse, the command returns an error:

   ```
   $ durable schedules create hourly-sync --cron "bad-expr" --def nightly-report
...
**Step 5: HostCalls threading verification.** For every function in the durable closure, the transformer verifies that `*durable.HostCalls` is available. The acceptable patterns are:

1. **Direct parameter:** `func foo(h *durable.HostCalls, ...)` — `h` is the first parameter.
...
error: shipping.go:15: function createShipment is in the durable closure
       (it calls DurableCall) but has no access to *durable.HostCalls.
...
func makeHostCalls(mem *wasmMemory) *durable.HostCalls {
...
**Step 8: Generate WASM exports.** For each workflow entry point (exported function whose first parameter is `*durable.HostCalls`), the transformer generates a `//go:wasmexport` function. These are the functions the host runtime calls to start or resume a workflow:
...
$ durable deploy ./workflows/place_order.wasm
...
| **7. Compilation + CLI** | `go build`/`tinygo build` integration, `durable build/deploy/test` CLI | 2 weeks |
...
**Design principles:**
1. **Same code as production.** The workflow function takes a `*durable.HostCalls` — the same struct whether it's the test host or the production WASM adapter. No build tags, no test-only code paths.
...
```go
func MonthlyBilling(h *durable.HostCalls, billingPeriod string) error {
...
This is essentially the current design with better tooling. The difference is in presentation: instead of "Go minus what WASM does not support," it is "durable Go — a verified subset of Go for durable execution." The error messages from `durable vet` become the developer's primary interaction with the restriction system, replacing the trial-and-error of "does this compile to WASM?" with authoritative, specific guidance.
...
```go
func PlaceOrder(h *durable.HostCalls, userID string) error {
