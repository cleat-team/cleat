...
var adapterDefs = map[string]adapterDef{
	...
	"PluginCall": {
		...
		ResultStmts: withSuspendCheck(
			...
			"errCode := uint32(result & 0xFF)",
			"if errCode != 0 {",
			`	return "", fmt.Errorf("plugin_call: %s", callErrorMessage("plugin_call", responseBuf, responseLen, callErrorCode))`,
			"}",
			"return unsafe.String(&responseBuf[0], int(responseLen)), nil",
		),
	},
	...
	"PluginCallStreaming": {
		...
		ResultStmts: withSuspendCheck(
			...
			"errCode := uint32(result & 0xFF)",
			"if errCode != 0 {",
			`		return nil, fmt.Errorf("plugin_call_streaming: %s", callErrorMessage("plugin_call_streaming", responseBuf, responseLen, callErrorCode))`,
			"}",
