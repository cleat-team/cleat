...
func GenerateExports(pkgName string, result *analyzer.AnalysisResult, target string) []byte {
	...
	// THE BODIES ARE EMITTED FIRST, so the imports and helpers can be decided
	// by what they ACTUALLY CONTAIN rather than by a second prediction of it.
	// cleat#1697.
	//
	// needsJSON used to be hasComplexParams(result) -- a separate walk of the
	// parameter types, guessing what the emitter downstream would do. The two
	// have now drifted TWICE, in opposite directions, and each time the
	// generated file compiled nowhere:
	//
	//   cleat#1036  ints moved onto json.Unmarshal; the predicate still called
	//               them simple -> "undefined: json"
	//   cleat#1065  strings moved onto extractJSONRaw; the predicate still
	//               called them simple -> "undefined: extractJSONRaw"
	//
	// And the obvious repair -- teaching the predicate that a string is complex
	// too -- breaks the OTHER direction: a lone-string entry point takes the
	// whole-payload fast path, emits no call, and would then carry an unused
	// `encoding/json` import, which is equally a compile error. There is no
	// value of the predicate that is right for both, because the question is
	// not "what types are declared" but "what did the emitter emit".
	//
	// Asking the text removes the second derivation entirely. A future change
	// to any binding arm cannot desynchronise this, because there is nothing
	// left to desynchronise.
	var body bytes.Buffer
	bodyQual := types.RelativeTo(result.TargetPkg.Types)
	for _, epName := range result.EntryPoints {
		fd := result.Funcs[epName]
		if fd == nil || fd.Type == nil {
			continue
		}
		generateExport(&body, fd, bodyQual, target)
	}
	if len(result.EntryPoints) > 0 {
		body.WriteString(deferRunnerExportSource)
	}
	if target == GoTarget {
		generateDispatch(&body, result, bodyQual)
	}
	...
	// Both are emitted by the same arms, but ask for each separately: a future
	// arm could use one without the other, and an unused import is as fatal as
	// a missing one.
	needsJSON := strings.Contains(generated, "json.Unmarshal(")
	needsExtractRaw := strings.Contains(generated, "extractJSONRaw(")
...
`)
	...
	// The bodies, generated at the top of this function.
	//
	// deferRunnerExportSource is emitted only for a module that HAS workflows:
	// a package with no entry points can register no defers, so a runner for
	// them would be an export with nothing to run, and
	// TestGenerateExportsEmptyEntryPoints pins the broader invariant that such
	// a package produces no callable surface at all.
	//
	// generateDispatch is the Go target only. The wasmtime backend calls
	// _start, which calls main(), which uses cleat_poll_work + cleatDispatch to
	// route work to entry points; the wazero backend calls exports directly.
	// Both paths coexist.
