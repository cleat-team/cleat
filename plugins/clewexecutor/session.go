...
func roleForPhase(phase string) (string, error) {
	...
	case "brief", "brief_delivered", "briefing":
		return "cto", nil // CTO lap brief phases
	case "done":
...
func taskDir(projectRoot, taskStatePath, taskID string) string {
	if taskStatePath == "" {
		taskStatePath = "task_state"
	}
	return filepath.Join(projectRoot, taskStatePath, taskID)
}
