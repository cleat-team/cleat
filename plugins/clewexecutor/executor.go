...
func buildPrompt(in runPhaseInput, role, td, protocolPath, currentPhase string) (string, error) {
	...
	case "developer":
		...
		case "create_pr":
			return fmt.Sprintf(
				"You are a developer agent with ONE job: create a clean, signed, conflict-free PR.\n\nProject: %s. Task ID: %s. Branch: '%s'.\n\nFirst read %s/TASK.md.\n\nSTEP 1 — Sign off your work (required for DCO, safe everywhere):\n  git commit --amend --signoff --no-edit\n\nSTEP 2 — Rebase onto latest develop:\n  git fetch origin develop\n  git rebase origin/develop\n  If conflicts: resolve each file, git add, git rebase --continue.\n  If catastrophic: git rebase --abort, then git merge origin/develop.\n\nSTEP 3 — Push:\n  git push --force-with-lease origin %s\n\nSTEP 4 — Create the PR:\n  gh pr create --base develop --head %s \\\n    --title \"<descriptive summary from TASK.md>\" \\\n    --body \"$(cat %s/artifacts/implementation.md 2>/dev/null || echo 'Implementation complete.')\"\n\nSTEP 5 — Verify:\n  gh pr view --json mergeable,url,statusCheckRollup\n  If CONFLICTING: go back to STEP 2.\n  If DCO check failed: go back to STEP 1.\n\nReport the PR URL and stop. DO NOT implement, review, or write artifacts.",
				in.Project, in.TaskID, in.TaskID, taskPath, in.TaskID, in.TaskID, in.TaskID, taskPath,
			), nil
		case "ci_fix":
			return fmt.Sprintf(
				"You are a developer agent with ONE job: fix CI failures on an existing PR.\n\nProject: %s. Task ID: %s. Branch: '%s'.\n\nSTEP 1 — Run 'gh pr checks' to see ALL failing checks.\n\nSTEP 2 — Fix DCO (if failing, do this FIRST):\n  git commit --amend --signoff --no-edit\n  git push --force-with-lease origin %s\n  Re-run 'gh pr checks'. If DCO passes, go to next step.\n\nSTEP 3 — Fix Build/Lint/Test failures (if any):\n  Read the failure log from the CI check details.\n  Fix the code. Run the build or tests locally to verify.\n  git add <fixed files>\n  git commit --signoff -m 'fix: resolve CI failures'\n  git push --force-with-lease origin %s\n\nSTEP 4 — Verify ALL checks pass with 'gh pr checks'.\n  If any still failing: go back to STEP 2 or STEP 3.\n\nDo NOT change the PR title/description. Do NOT close and re-create the PR. Do NOT implement new features.",
				in.Project, in.TaskID, in.TaskID, in.TaskID,
			), nil
		case "rebase_fix":
			return fmt.Sprintf(
				"You are a developer agent with ONE job: resolve merge conflicts on an existing PR.\n\nProject: %s. Task ID: %s. Branch: '%s'.\n\nSTEP 1 — Fetch and rebase:\n  git fetch origin develop\n  git checkout %s\n  git rebase origin/develop\n\nSTEP 2 — For EACH conflict:\n  Open the file, understand both sides, merge correctly.\n  git add <resolved-file>\n  git rebase --continue\n  If rebase fails badly: git rebase --abort, then git merge origin/develop.\n\nSTEP 3 — Sign off and push:\n  git commit --amend --signoff --no-edit\n  git push --force-with-lease origin %s\n\nSTEP 4 — Verify:\n  gh pr view --json mergeable,url,statusCheckRollup\n  If still CONFLICTING: go back to STEP 1.\n  If DCO check failed: go back to STEP 3.\n\nReport 'Resolved' and stop. DO NOT implement features or review code.",
				in.Project, in.TaskID, in.TaskID, in.TaskID, in.TaskID,
			), nil
		case "merge":
			return fmt.Sprintf(
				"You are a developer agent with ONE job: merge an approved PR.\n\nProject: %s. Task ID: %s.\n\nSTEP 1 — Find the PR:\n  gh pr list --head %s --json number,mergeable,url,statusCheckRollup\n  If no PR found: report and stop.\n\nSTEP 2 — Ensure CI green and branch current:\n  git fetch origin develop\n  git checkout %s\n  git rebase origin/develop\n  If conflicts: resolve, git add, git rebase --continue.\n  git commit --amend --signoff --no-edit\n  git push --force-with-lease origin %s\n\nSTEP 3 — Merge:\n  gh pr merge <NUMBER> --squash --delete-branch\n  (If --squash fails, try --merge)\n\nSTEP 4 — Update status:\n  Edit %s/STATUS.md: change **Phase:** merge to **Phase:** done\n\nReport 'Merged — task done' and stop.",
