...
func (p *Plugin) handleCreateConfig(w http.ResponseWriter, r *http.Request) {
	...
	`, p.dialect), tid, id, req.Name, req.Cron, req.S3Bucket, req.S3Prefix, retentionDays, enabled, next)
	if err != nil {
...
func (p *Plugin) handleDeleteConfig(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = p.db.Exec(r.Context(), plugin.Rebind(`DELETE FROM backup_history WHERE config_id = $1 AND tenant_id = $2`, p.dialect), id, tid)
...
func (p *Plugin) handleRunBackup(w http.ResponseWriter, r *http.Request) {
	...
	`, p.dialect), historyID, id, tid, filename, now)
	if err != nil {
