...
public class TestHostCalls {
    ...
    public DurableResult<String> durableCall(String service, String operation, String requestJSON) {
        ...
        for (int i = 0; i < callStubs.size(); i++) {
            ...
            if (stub.service.equals(service) && stub.operation.equals(operation)) {
                ...
                if (err != null) {
                    return DurableResult.err(err);
                }
                return DurableResult.ok(resp);
            }
        }
    ...
    public DurableResult<String> pollSignal(String signalName) {
        for (int i = 0; i < pendingSignals.size(); i++) {
            ...
            if (sig.name.equals(signalName)) {
                ...
                return DurableResult.ok(sig.payload);
            }
        }
    ...
    public DurableResult<String> awaitChild(String runID) {
        ...
        if (result != null) {
            return DurableResult.ok(result);
        }
        return DurableResult.ok("{\"status\":\"completed\"}");
    }

    /**
     * Wait for one or more external signals with a timeout.
     */
    public DurableResult<HostCalls.AwaitSignalsResult> awaitSignals(
            String[] signalNames, long timeoutMs) {
        for (int i = 0; i < pendingSignals.size(); i++) {
            PendingSignal sig = pendingSignals.get(i);
            if (contains(signalNames, sig.name)) {
                pendingSignals.remove(i);
                return DurableResult.ok(
    ...
    public DurableResult<String> getQueryState(String key) {
        ...
        if (val != null) {
            return DurableResult.ok(val);
        }
        return DurableResult.err("query state key not found: " + key);
    }
     ...
     */
    public DurableResult<HostCalls.AwaitPromiseResult> awaitPromise(
            String id, long timeoutMs) {
        String status = promises.get(id);
        if (status == null) {
            return DurableResult.err("promise not found: " + id);
        }

        if ("resolved".equals(status)) {
            String val = promiseResults.getOrDefault(id, "");
            return DurableResult.ok(new HostCalls.AwaitPromiseResult(val, false));
        }
        ...
        nowMs += timeoutMs;
        return DurableResult.ok(new HostCalls.AwaitPromiseResult("", true));
    }
     ...
     */
    public DurableResult<Void> scheduleInvoke(String service, String operation,
    ...
    public DurableResult<String> getState(String key) {
        ...
        return DurableResult.err("no such key: " + key);
    }
    ...
    public DurableResult<String> listState(String prefix) {
        ...
        sb.append("]");
        return DurableResult.ok(sb.toString());
    }
    ...
    public DurableResult<String> sendSignalAndWait(
        ...
        if (!"__pending__".equals(reply)) {
            ...
            return DurableResult.ok(reply);
        }

        // Simulate timeout
        nowMs += timeoutMs;
        return DurableResult.err("SendSignalAndWait timed out");
    }
    ...
    public DurableResult<Void> replyToSignal(String correlationId, String response) {
        ...
        return DurableResult.err("no pending signal for correlation ID: " + correlationId);
    }

    /**
     * Send a signal to a target workflow (fire-and-forget).
     */
    public DurableResult<Void> signalWorkflow(
