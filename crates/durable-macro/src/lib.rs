...
/// Marks a function as a cleat durable workflow entry point.
///
...
///     let (reservation_json, err) = h.durable_call("inventory", "Reserve", &serde_json::to_string(&input).unwrap());
///     // ...
///     Ok("done".to_string())
/// }
/// ```
...
pub fn durable_entry(_attr: TokenStream, item: TokenStream) -> TokenStream {
    entry::durable_entry_impl(item.into()).into()
}
