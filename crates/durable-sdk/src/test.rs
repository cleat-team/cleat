...
//!
//! ```rust
//! use durable_sdk::test::{DurableTest, CallRecord};
//!
//! #[test]
//! fn test_payment_workflow() {
//!     let mut env = DurableTest::new();
//!
...
//!         let outcome = h.durable_call("payment", "charge", input);
...
impl MockHostCalls {
    ...
    /// Returns `(response_json, error_message)` matching the Rust SDK's `HostCalls::durable_call`.
    pub fn durable_call(&mut self, service: &str, operation: &str, request_json: &str) -> (String, Option<String>) {
        // Retry simulation
    ...
    pub fn durable_sleep(&mut self, duration_ms: i64) -> bool {
    ...
    pub fn durable_log(&self, _message: &str) {
        // No-op for testing
    }
    ...
    pub fn durable_defer(&mut self, _description: &str) -> (String, Option<String>) {
    ...
    pub fn durable_send(&mut self, service: &str, operation: &str, request_json: &str) -> Result<(), String> {
        self.call_history.push(CallRecord {
            service: service.to_string(),
            operation: operation.to_string(),
            request: request_json.to_string(),
...
// ═════════════════════════════════════════════════════════════════════════════
// DurableTest — high-level test environment orchestrator
// ═════════════════════════════════════════════════════════════════════════════
...
///
/// ```rust
/// use durable_sdk::test::DurableTest;
///
/// #[test]
/// fn test_workflow() {
///     let mut env = DurableTest::new();
...
///             let (resp, err) = host.durable_call("payment", "charge", input);
///             if let Some(e) = err {
...
/// ```
#[derive(Debug, Clone)]
pub struct DurableTest {
    /// The underlying MockHostCalls instance.
    pub mock: MockHostCalls,
}

impl DurableTest {
    /// Create a new DurableTest with clean state.
    pub fn new() -> Self {
        Self {
...
mod tests {
    ...
    fn test_call_stub_and_recording() {
        ...
        let (resp, err) = env.mock.durable_call("payment", "charge", r#"{"amount":5000}"#);
        assert!(err.is_none());
    ...
    fn test_no_stub_error() {
        let mut env = DurableTest::new();
        let (resp, err) = env.mock.durable_call("unknown", "op", r#"{}"#);
        assert!(resp.is_empty());
    ...
    fn test_assert_called() {
        ...
        env.mock.durable_call("svc", "op", r#"{}"#);
    ...
    fn test_retry_simulation() {
        ...
        let (resp1, err1) = env.mock.durable_call("payment", "charge", r#"{"amount":5000}"#);
        assert!(resp1.is_empty());
        assert!(err1.unwrap().contains("simulated transient failure"));

        let (resp2, err2) = env.mock.durable_call("payment", "charge", r#"{"amount":5000}"#);
        assert!(resp2.is_empty());
        assert!(err2.unwrap().contains("simulated transient failure"));

        // Third call should succeed
        let (resp3, err3) = env.mock.durable_call("payment", "charge", r#"{"amount":5000}"#);
        assert!(err3.is_none());
    ...
    fn test_run_workflow_closure() {
        ...
        let result = env.run_workflow(
            |host: &mut MockHostCalls, input: &str| -> String {
                let (resp, _err) = host.durable_call("greeter", "hello", input);
                resp
            },
            r#"{"name":"test"}"#,
        );
    ...
    #[test]
    fn test_durable_send_recording() {
        let mut env = DurableTest::new();
        env.mock.durable_send("notification", "email", r#"{"to":"user@example.com"}"#).unwrap();
