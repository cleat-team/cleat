...
func deployFixture(t *testing.T, db *sql.DB, taskQueue string) {
	...
	if _, err := db.Exec(`
		...
		VALUES ('crashcall', 1, $1, ARRAY['three_charges','compensating','with_cleanup','continues_as_new','cleanup_with_backoff','parent_with_child','child_echo'], 1, 10000, '{}'::jsonb, $2, 1, '{}'::jsonb, $3)
