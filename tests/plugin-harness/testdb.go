...
//
// For MSSQL this delegates to migration.SplitMSSQL, the production splitter
// used by migration.Runner, which splits only on GO batch separators. This
// harness used to run MSSQL SQL through the same generic semicolon splitter
// as every other dialect (see splitSQL below), which also cut on every
// semicolon inside a batch -- including the ones inside a stored procedure
// body -- and broke migrations/mssql/003_procedures.sql's CREATE OR ALTER
// PROCEDURE into fragments the server rejected with "Incorrect syntax near
// 'ON'". The production splitter already gets this right (see
// migration/runner.go); the harness only needed to call it.
//
