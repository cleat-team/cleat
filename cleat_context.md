...
**Multi-instance by design:** Run several independent cleat worker pools against the same PostgreSQL cluster. Each pool gets its own schema via the `--schema` flag. Pools can cooperate across schemas via `--peer-schemas` and `cleat_child_workflow_in_schema`. There is no centralized cleat control plane — just PostgreSQL and your worker processes.
