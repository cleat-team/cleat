...
| file | protocol |
|---|---|
| `IMPROVEMENT-PLAN.md` | Edit only your own `§` sections. **Do not pick a number by hand — run `scripts/next-section-number.sh`.** Sections are allocated in per-stream blocks (WS-1 `3.200–299`, WS-2 `3.300–399`, WS-3 `3.400–499`), enforced by `scripts/check-section-numbers.sh`. The earlier §3.10/3.20/3.30 allocation was breached and retired (#563, 2026-09-02), and its replacement — "take the next free number above the highest in the file" — is the move that collided; it is what the blocks exist to stop. |
...
| item | what |
|---|---|
| **§3.12 / §3.77** | ✅ **DONE 2026-09-03. D7 is implemented end to end.** All three tables that key on a user-chosen name now carry the tenant: `workflow_defs` (postgres 035 / mysql 034 / mssql 038, #594), `workflow_schedules` (036/035/039, #601) and `workflow_tags` (037/036/040, #605). **The migration hazard this row used to warn about is gone rather than solved**: it was that `instance.tenant_id != def.tenant_id` was legal via the default-tenant adoption window, so widening the foreign key would break existing rows. The owner's answer was that there are no existing workflows to preserve, so option 4 -- change the key outright -- applied, and the adoption window (`canAdoptDef`, `ErrWorkflowDefOwnedByAnotherTenant`, `engine/def_ownership.go`, the 409 arm) was **deleted** rather than carried forward. The old estimate of “~96 query sites, a multi-session item” was wrong in a useful direction: most sites already carried `tenant_id` from §3.10/§3.11, and the expensive part was the migration. **What the work exposed is now the open item, not this**: §3.86, where SQL Server's `dbo.fn_tenant_filter` is disabled outright for any `dbo.cleat_admin` connection -- which a multi-tenant deployment must use -- so 23 statements over these tables had no tenant predicate of their own. Fixed for schedules, tags and definitions (#597, #604, #607) and for the five control-plane statements whose id arrives from an HTTP request; **27 remain**, and they are now the allowlist of `engine/mssql_tenant_predicate_test.go` rather than a count to re-derive. Those 29 are characterised rather than leftover -- all plumbing, running on stores `cmd/cleat-worker/setup.go:storeFor` re-scopes per instance, so they are scoped by construction and are deliberately not swept. The open item is the gate, and its allowlist has to say *that*, not "a UUID cannot be guessed". |
| **§3.15** | ✅ **DONE 2026-09-02 (#570).** The writer exists: `SetAllowedSignalCallers` on three dialects and `GET`/`PUT /api/workflows/:id/allowed-signals`. The flag still defaults to `false`, for a reason that outlived the missing writer — nothing sets the list at workflow *start*, so enabling it denies every signal until an operator grants callers per workflow. Start-time declaration is the follow-up and is a product call. ~~🔴 Open, and it is a live denial rather than a latent gap.~~ `GetAllowedSignalCallers` reads `workflow_instances.allowed_signals`, and nothing in the product writes it — no store method, no endpoint, no CLI verb, no SDK call. The consumer is installed whenever `--require-signal-auth` is set. **The bleeding is stopped, correctly and on both sides** — the flag defaults to `false` with the item named in its own help text (`cmd/cleat-worker/config.go:90`), and `docs/reference/worker-config.md:197` opens the section with "**Not usable yet.** Nothing in cleat can write `allowed_signals`". So this is no longer a trap; it is an advertised flag that cannot be turned on. That makes it the cleanest piece of *new feature* work on either board rather than a defect to fix: a write path (store method → API → CLI), after which the flag's default becomes a decision. |
...
> | | item | why here |
> |---|---|---|
> | ~~1~~ | ~~**the defer-phase record shape** (WS-2, §3.35 phase 5)~~ | **Done, 2026-09-02 (§3.75, #569)** — answered within hours of reaching the board, which is the argument for sequencing by who is blocked rather than by cost. WS-3 is unblocked. |
> | ~~2~~ | ~~**§3.15, the signal-authorization write path** (WS-1)~~ | **Done, 2026-09-02 (#570).** The writer exists; the flag stays off until a workflow can declare its callers at start time. |
> | **1** | **§2.35**, `error_code` per event (WS-2) | Now the top item: the substantial engineering work on either board that is not blocked on a product decision. §3.75 settled the record question it would otherwise have had to answer twice. |
>
