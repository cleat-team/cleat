	...
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			for i := 0; i < 5; i++ {
				...
				_, alreadyExisted, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{}`), key)
				if err != nil {
...
func TestClaimStickyWorkflows(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			runID, _, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{}`), "")
			if err != nil {
...
func TestExactlyOnceStart(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			runID1, alreadyExisted, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{}`), "idem-1")
			if err != nil {
			...
			runID2, alreadyExisted, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{}`), "idem-1")
			if err != nil {
...
func TestExactlyOnceStart_DifferentKeys(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			runID1, alreadyExisted1, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{}`), "idem-a")
			if err != nil {
			...
			runID2, alreadyExisted2, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{}`), "idem-b")
			if err != nil {
...
func TestCompleteWorkflow(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			result := `{"done":true}`
			if err := store.CompleteWorkflow(ctx, wf.ID, "worker-1", result, map[string]string{"key": "val"}); err != nil {
...
func TestFailWorkflow(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			_, _, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{}`), "")
			if err != nil {
				t.Fatalf("StartNewRun: %v", err)
			}
			...
			errCode := "ERR_001"
			errOp := "test-op"
			if err := store.FailWorkflow(ctx, wf.ID, "worker-1", errMsg, errCode, errOp, nil); err != nil {
...
func TestReleaseWorkflow(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			if err := store.ReleaseWorkflow(ctx, wf.ID, "worker-1", nextWakeAt); err != nil {
...
func TestContinueAsNew_Atomic(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			_, _, err := store.StartNewRun(ctx, "", "test-workflow", 1, json.RawMessage(`{"v":1}`), "")
			if err != nil {
				t.Fatalf("StartNewRun: %v", err)
			}
...
func TestFinalizeWorkflowSegment(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			if err := store.FinalizeWorkflowSegment(ctx, wf.ID, "worker-1", events, "done", `{"done":true}`, "", "", nil, time.Time{}); err != nil {
...
func TestHeartbeat(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			owned, err := store.Heartbeat(ctx, wf.ID, "worker-1")
			if err != nil {
			...
			owned, err = store.Heartbeat(ctx, wf.ID, "worker-2")
			if err != nil {
