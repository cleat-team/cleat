...
func TestGetQueryState(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			err = store.CompleteWorkflow(ctx, target.ID, "qstate-worker",
...
func TestDeleteExpiredEvents(t *testing.T) {
	for _, backend := range registeredBackends {
		...
		t.Run(backend.Name(), func(t *testing.T) {
			...
			// (expired) candidate.
			runID, _, err := store.StartNewRun(ctx, "", "test-workflow", 1,
				json.RawMessage(`{}`), "expired-test-1")
			if err != nil {
				t.Fatalf("StartNewRun: %v", err)
			}
			...
			err = store.CompleteWorkflow(ctx, target.ID, "expired-worker",
				`{"result":"done"}`, nil)
			if err != nil {
				t.Fatalf("CompleteWorkflow: %v", err)
			}
