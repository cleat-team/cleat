-- ===========================================================================
-- 065: workers are enumerable (workers)
--
-- Why this exists
-- ---------------
...
-- that belongs to one tenant. MySQL has no schemas, so like tenant_api_keys it is unprefixed; for
-- that reason. It gets no row-level security policy, in the same sense and for
-- the same reason that `tenants` gets none: it is global BY DESIGN, not
...
CREATE TABLE IF NOT EXISTS workers (
    ...
    started_at        TIMESTAMP(6)  NOT NULL DEFAULT NOW(6),
    last_heartbeat_at TIMESTAMP(6)  NOT NULL DEFAULT NOW(6),
    PRIMARY KEY (worker_id),
    INDEX idx_workers_last_heartbeat (last_heartbeat_at)
) ENGINE=InnoDB;
