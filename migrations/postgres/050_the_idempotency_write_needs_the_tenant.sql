...
--
-- The two terminal-status UPDATEs matched on workflow_id alone:
--
...
-- rather than an attack". So tenant B finalizing its workflow overwrote every
-- tenant's idempotency row naming that workflow_id.
--
...
-- (cmd/cleat-worker/setup.go:1935), which delegates to THIS procedure.
-- PostgresStore.CompleteWorkflow's only non-test callers are in cmd/cleat-bench.
-- That change is still correct for the paths that do use it; it simply was not
-- on the path production takes. A Go-level grep cannot see this statement at
-- all -- it is SQL, in a procedure body, in a migration.
--
-- THE TENANT COMES FROM THE ROW, not from a new parameter. The procedure's
-- signature is unchanged, so no Go caller changes. workflow_instances.id is a
-- bare primary key, so the subquery names exactly the workflow being finalized
-- and therefore exactly the tenant that owns it.
--
-- Replaces the procedure whole, so this file otherwise reproduces
-- migrations/postgres/049_a_burst_wakes_finalize_on_progress.sql verbatim.
-- Only the two idempotency_keys predicates change.
