-- ===========================================================================
-- 071: a tenant says which hosts its workflows may reach (admin.tenant_egress_allow)
--
...
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON s.schema_id = t.schema_id
               WHERE s.name = 'admin' AND t.name = 'tenant_egress_allow')
BEGIN
    CREATE TABLE admin.tenant_egress_allow (
        tenant_id  UNIQUEIDENTIFIER NOT NULL,
        host       NVARCHAR(255)    NOT NULL,
        created_at DATETIMEOFFSET   NOT NULL DEFAULT SYSUTCDATETIME(),
        CONSTRAINT pk_tenant_egress_allow PRIMARY KEY (tenant_id, host),
        CONSTRAINT fk_tenant_egress_allow_tenant FOREIGN KEY (tenant_id)
