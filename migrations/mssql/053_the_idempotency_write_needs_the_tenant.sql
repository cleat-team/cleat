-- cleat#1017: finalize_workflow_status wrote idempotency_keys across tenants.
--
-- The two terminal-status UPDATEs matched on workflow_id alone. idempotency_keys
-- has NO row-level security on any dialect, so that predicate was the whole of
-- the protection and it did not mention the tenant. A workflow_id is
-- caller-supplied when a run id is given, and migration 010 already records the
-- collision as ordinary rather than adversarial -- "two customers both choosing
-- 'order-123' collided... the expected outcome of ordinary naming rather than an
-- attack". So tenant B finalizing its workflow overwrote every tenant's
-- idempotency row naming that workflow_id.
--
-- WHY THE GO-LEVEL FIX DID NOT COVER THIS. #1012 added AND tenant_id to
-- CompleteWorkflow, FailWorkflow and MoveToDeadLetterQueue in all three stores.
-- The worker calls none of them: it finalizes through FinalizeWorkflowSegment
-- (cmd/cleat-worker/setup.go:1935), which delegates to THIS procedure. Those
-- store methods' only non-test callers are in cmd/cleat-bench. A Go-level grep
-- cannot see this statement at all -- it is SQL, in a procedure body, in a
-- migration.
--
-- THE TENANT COMES FROM THE ROW, not from a new parameter, so the signature is
-- unchanged and no Go caller changes. workflow_instances.id is a bare primary
-- key, so the subquery names exactly the workflow being finalized and therefore
-- exactly the tenant that owns it.
--
-- Replaces the procedure whole, so this file otherwise reproduces
-- migrations/mssql/052_a_burst_wakes_finalize_on_progress.sql verbatim. Only the two idempotency_keys predicates change.
...
--
-- Replaces the procedure whole, so this file otherwise reproduces migrations/mssql/050_a_signal_delivered_mid_segment_wakes_the_workflow.sql
