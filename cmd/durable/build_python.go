...
func runBuildPython(pattern, outDir string) {
	...
	if err := wasm.BuildPythonWasm(entry, wasmOutput, false); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Make sure the Python file has a @durable_entry decorated function.\n")
		os.Exit(1)
	}
...
// detectEntryFunction scans a .py file for the first @durable_entry decorated
...
func detectEntryFunction(pyFile string) (string, error) {
	...
	for i, line := range lines {
		...
		if strings.HasPrefix(trimmed, "@durable_entry") {
	...
	return "", fmt.Errorf("no @durable_entry decorated function found in %s", pyFile)
}
