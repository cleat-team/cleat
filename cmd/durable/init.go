...
func runInit(args []string) {
	...
	if flags.NArg() < 1 {
		fmt.Fprintf(os.Stderr, "Usage: durable init [--template agent|basic] <project-name>\n")
		os.Exit(1)
	}
...
// @durableEntry(name="hello")
func Hello(h durable.HostCalls, input string) (string, error) {
...
func writeYAML(dir, projectName string) {
	...
	os.WriteFile(filepath.Join(dir, "durable.yaml"), []byte(yamlContent), 0644)
}
