...
func executeTool(h durable.HostCalls, name, argsJSON string) string {
	switch name {
	case "calculator":
	...
	case "web_search":
		...
		json.Unmarshal([]byte(argsJSON), &args)
		result := fmt.Sprintf("Web search for '%s' would execute here via a durable HTTP call to a search API.", args.Query)
