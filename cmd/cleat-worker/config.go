...
var (
	...
	hostRetryBudget                = flag.Duration("host-retry-budget", engine.DefaultHostRetryBudget, "CEILING on how much worst-case backoff a retry policy may carry and still be run on the host, inside one segment, holding the worker slot. A policy above this is refused (callErrorCode 6, RetryPolicyTooLong); the guest then runs it itself, suspending between attempts, which releases the slot. A tenant may set a LOWER value in tenant_settings and it is clamped to this; it can never raise it. Keep it well below --wasm-wall-clock-ceiling: that ceiling covers the whole invocation, so a budget near it lets one retry policy consume everything the workflow had. 0 uses the built-in default.")
