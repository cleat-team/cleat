...
var (
	...
	wasmInstanceTimeout           = flag.Duration("wasm-instance-timeout", 30*time.Second, "Max wall-clock time for a single WASM invocation (one fresh execution or one replay pass) before it is forcibly interrupted. Enforced via wasmtime epoch interruption on the wasmtime backend, which bounds even a WASM module stuck in a tight loop that never calls back into the host. On the wazero backend (the CGO-less fallback; see --allow-wazero-fallback) this deadline is NOT enforced against a compute-bound guest: wazero has no working mid-execution interruption mechanism (CLAUDE.md), so a tight loop that never yields to the host keeps running past this timeout. This is the primary bound against a runaway workflow hanging a worker on the wasmtime backend only; 0 disables it and is NOT recommended.")
	allowWazeroFallback           = flag.Bool("allow-wazero-fallback", false, "Permit starting with the wazero WASM backend when this is a CGO-enabled binary and wasmtime (the backend of record) failed to initialize for a reason other than CGO being disabled. Without this flag, that condition is fatal at startup, because wazero cannot fence a compute-bound guest (CLAUDE.md) and silently downgrading to it would be a safety-relevant degradation. Does NOT affect binaries built with CGO_ENABLED=0, which use wazero by design and log a WARN, not an error, on startup.")
