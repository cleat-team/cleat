...
// markCompletedAnHourAgo marks a run terminal with a completed_at an hour in the
// past, on the DATABASE's clock.
//
// THIS IS HARDENING, NOT A DIAGNOSIS, and the distinction is the point of this
// comment. It replaces `completed_at = now()` paired with `older_than: "1ns"`,
// which compares two independently-sourced clocks across a one-nanosecond
// margin: the row's timestamp is generated by the database and the sweep's
// cutoff is `time.Now().Add(-window)` in the worker process. Nothing in the
// test establishes a relationship between those two readings, and a
// one-nanosecond margin needs one.
//
// The nanosecond buys no coverage. What both tests actually rest on is the
// CONTRAST between a window an override can express and the 30-day one the
// configuration can: a run completed an hour ago is four hundred times inside
// the configured window and comfortably outside a one-second override, so every
// assertion here is unchanged and none of them is now a race.
//
// WHAT PROMPTED IT, stated in full because the story does not end in an
// explanation. TestTheSweepDeletesOnlyWithTheOverride/override_reaches_it
// failed twice on 2026-09-13 against a containerised PostgreSQL 16 -- on this
// branch and, run identically, on unmodified develop, so not a regression. It
// then passed 52 consecutive times and has not recurred.
//
// The first explanation offered for it was clock skew, and that was WRONG: the
// measurement behind it timed `docker exec … psql` sequentially and reported
// the tool's own startup latency (+0.489s) as skew. Bracketing the query
// between two host readings puts the database clock inside the bracket on every
// sample. Four hypotheses were tested and all four are refuted --
//
//	container clock skew   no skew, including on a container sampled from boot
//	-race                  passes with and without
//	all three DSNs set     passes either way
//	fresh suite database   dropped cleat_test_cleat_worker, ×3, passes
//
// -- so the cause is unknown, and this change does not address it. It is
// recorded as its own issue with the evidence. If that failure returns, the
// margin here is not what is wrong.
//
// The sibling engine test solves the same shape from the other side, with a
// cutoff an hour in the FUTURE (retention_deletes_the_idempotency_key_test.go,
// which writes out the same reasoning). That is unavailable here: this endpoint
// deliberately rejects a non-positive older_than -- the "zero" and "negative"
// cases above assert exactly that -- so reaching a fresh row through the HTTP
// API would require the request the API refuses. The row moves instead of the
// cutoff.
const markCompletedAnHourAgo = `UPDATE workflow_instances
	SET status = 'done', completed_at = now() - interval '1 hour' WHERE id = $1`
...
func TestTheSweepDeletesOnlyWithTheOverride(t *testing.T) {
	...
	newCompletedRun := func(t *testing.T) string {
		...
		// Terminal, and completed an HOUR ago rather than now(). Set through the
		// same columns DeleteCompletedWorkflows selects on, against the schema
		// the migrations built -- not a table invented for this test.
		//
		// The hour is what makes this test independent of the two clocks. See
		// markCompletedAnHourAgo's comment.
		if _, err := db.ExecContext(ctx, markCompletedAnHourAgo, id); err != nil {
	...
	t.Run("no override cannot reach a fresh run", func(t *testing.T) {
		...
		if !exists(t, id) {
			t.Error("the configured sweep deleted a run that completed an hour ago. " +
	...
	t.Run("override reaches it", func(t *testing.T) {
		id := newCompletedRun(t)
		if got := sweepReq(t, s, `{"older_than":"1s"}`).Code; got != 200 {
			t.Fatalf("sweep answered %d, want 200", got)
		}
		if exists(t, id) {
			t.Error("the run survived a sweep with older_than=1s against a run that completed an hour ago. Without a window " +
				"override the endpoint can never reach anything inside the configured " +
				"window, on any call, while reporting success -- a feature that ships working " +
...
func TestAnOverrideDoesNotEnableADisabledArm(t *testing.T) {
	...
	rec := sweepReq(t, s, `{"older_than":"1s"}`)
	if rec.Code != 200 {
