// Command durable-bench is a performance benchmark tool for cleat workers.
// It measures throughput, latency, and replay performance against a real
// PostgreSQL database.
//
// Usage:
//
//	durable-bench --db "postgres://..." --workflow <name> --count 100
package main
...
func main() {
	...
	if *dbURL == "" || *workflowName == "" {
		fmt.Fprintf(os.Stderr, "Usage: durable-bench --db <url> --workflow <name> [--count 100] [--concurrency 10]\n")
		os.Exit(1)
	}
	...
	if len(versions) == 0 {
		log.Fatalf("Workflow %q not found. Deploy it first with: durable deploy <wasm-file>", *workflowName)
	}
	version := versions[0]
