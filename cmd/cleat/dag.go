...
func dagUsage() {
	fmt.Fprintf(os.Stderr, "Usage: cleat dag <validate|run|generate> [options] <spec.json>\n\n")
...
func runDagValidate(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: cleat dag validate <spec.json>\n")
		os.Exit(1)
	}
...
func runDagRun(args []string) {
	...
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: cleat dag run [--input <json>] [--output <file>] <spec.json>\n")
		os.Exit(1)
	}
	...
	fmt.Printf("  cleat build -o ./out ./%s\n", filepath.Dir(outputPath))
}
...
func runDagGenerate(args []string) {
	...
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: cleat dag generate [--output <file>] <spec.json>\n")
		os.Exit(1)
	}
...
func generateDevProgram(spec *dagplugin.DAGSpec, inputJSON string) []byte {
	...
	b.WriteString("// Code generated by \"cleat dag run\"; DO NOT EDIT.\n\n")
...
func generateWorkflowFile(spec *dagplugin.DAGSpec) []byte {
	...
	b.WriteString("// Code generated by \"cleat dag generate\"; DO NOT EDIT.\n\n")
	b.WriteString("package main\n\n")
	b.WriteString("import (\n")
	b.WriteString("\t\"encoding/json\"\n")
	b.WriteString("\t\"fmt\"\n")
	...
	fmt.Fprintf(&b, "func %s(h cleat.HostCalls, input Input) (string, error) {\n", pipelineName)
	b.WriteString("\td := dagplugin.NewDAG()\n")
	for _, task := range spec.Tasks {
		parents := formatParents(task.Parents)
		fmt.Fprintf(&b, "\td.AddTask(%q, %s, %s)\n", task.Name, parents, task.Fn)
