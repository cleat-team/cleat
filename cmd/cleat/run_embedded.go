...
func runEmbedded(args []string) {
	...
	if len(remainder) == 0 && *wasmFile == "" {
		fmt.Fprintf(os.Stderr, "Usage: cleat run [flags] <package-path>\n")
		fmt.Fprintf(os.Stderr, "       cleat run --wasm <file.wasm> [flags]\n")
	...
	if *wasmFile != "" {
		...
		// Build then load.
		pkgPath := remainder[0]
		outDir, err := os.MkdirTemp("", "cleat-run-**")
		if err != nil {
		...
		// Build the workflow.
		runBuild(pkgPath, outDir, *target, "", "", false, false, false, false, 1)
