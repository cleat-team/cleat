...
harness that has only ever passed is a claim, not a check.
...
[#1280](https://github.com/cleat-team/cleat/pull/1280) with kvstore as its one adopter; the rest is
blocked on a decision recorded as [#1278](https://github.com/cleat-team/cleat/issues/1278).

**What tracing the data path changed about this entry.** It read "policies on the plugin tables
that already carry `tenant_id`", which is the obvious shape and is backwards. Plugins hold a bare
`*sql.DB` rather than the store, so `cleat.tenant_id` is unset on every plugin connection and
`beginTxWithRLS` never touches their statements. Policies added first would not catch a leak —
they would refuse the next plugin query. The adapter has to set the tenant *first*, where it is
inert because nothing reads the value yet.

**And the blocker for the remaining plugins is not the policies.** A tenant reaches a plugin on
exactly one path, the HTTP middleware. Host calls and background loops carry none, so for any
plugin with a cross-tenant sweep, "add a fail-closed policy" and "silently empty the sweep" are
the same change. kvstore is the only plugin whose every access site is request-scoped. #1278 holds
the classification and the two questions that have to be answered before the others follow.

Re-derive rather than quoting the table count here — it is a census of a growing population, and
the figure in the issue body and the one I measured disagree (21-of-25 against 27-of-29) without
either being load-bearing. The finding that matters is a predicate and does not drift: **no plugin
table had a policy on any dialect.**

Still open under this decision: whether to wire up `RegisterPluginTables` or delete it along with
the `grant_plugin_to_tenant` half that depends on it — both have zero callers, and
[#1279](https://github.com/cleat-team/cleat/issues/1279) records that `plugin-security.md`
documents the schema-per-tenant model they implement as though it were live. And `blob_content`,
