...
rather than a redaction rule. `PluginCall` records `PluginInput: inputJSON` and
separately hands that same string to the plugin function. Substitution happens
*inside* a wrapper around that function, so the recorder cannot observe it.
