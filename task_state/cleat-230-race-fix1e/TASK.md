# cleat-230-race-fix1e — Extend stdout/stderr race fix to executeComponent path
...
In `Engine.executeComponent()` (engine.go:1338), replace the `e.rt.InstantiateModuleNamed()` call at line 1453 with `e.rt.instantiateModuleNamedWithWriters()` using per-execution `bytes.Buffer` instances.
